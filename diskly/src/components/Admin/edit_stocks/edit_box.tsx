...
export default function Edit_CD({game_id, cancelbtn}:more_boilerplate_because_reactjs_moment){

  const [game_detail, setGame_Detail] = useState<game_detail_model | null>();
  ...
  async function fetchGameDetail(){
    const res = await fetch(`${API_URL}/admin/view-game-detail?id=${game_id}`);
    ...
    setImageSrc(`${API_URL}/${ data.cover_image_path }`)
  }
  ...
  async function updateGameCD(e:any){
    setLoading(true)
    e.preventDefault();
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/80 z-50 p-4" onClick={cancelbtn}>
      ...
      {loading && (
        ...
        <form className="rent-form flex flex-col gap-6" onSubmit={(e) => {updateGameCD(e)}}>
