...
export default function DenyTransaction({id, cancelbtn, refetchRentalData}:more_boilerplate_because_reactjs_moment){

async function approve_rental(id: number){
  const res = await fetch(`${API_URL}/admin/deny-transaction?id=${id}`, {
    method: "POST",
    credentials: "include"
  });
  if (!res.ok){ console.log("failed to deny")}
  refetchRentalData()
  cancelbtn()
}
  return(
