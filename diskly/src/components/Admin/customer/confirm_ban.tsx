...
export default function BanUser({id, username, cancelbtn, refetchRentalData}:more_boilerplate_because_reactjs_moment){

async function ban_user(id: number){
  const res = await fetch(`${API_URL}/admin/ban-user?id=${id}`, {
    method: "POST",
    credentials: "include"
  });
