...
export default function User_completed({userid}:more_boilerplate_because_reactjs_moment){
  const [userRentals, setUserRentals] = useState<TransactionType[]>([]);

  async function fetchData(){
    const res = await fetch(`${API_URL}/user/user-completed?user_id=${userid}&`);
        ...
        userRentals.map(item => (
          <div className="uprof-card" key={item.id}>
            ...
            <div className="uprof-card-info">
              ...
              {item.return_on && <p>Returned On: {formatDate( item.returned_on )}</p>}
              <span
                className={`
              uprof-status
              bg-green-500
